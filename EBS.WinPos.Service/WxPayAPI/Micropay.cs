...
{
   public class Micropay:IMicropay
    {
       ...
       public void Run(string orderCode, string body, int total_fee, string auth_code)
        {
            ...
            data.SetValue("out_trade_no", orderCode);//产生随机的商户订单号
            ...
            if (!result.IsSet("return_code") || result.GetValue("return_code").ToString() == "FAIL")
            {
               ...
               // throw new WxPayException("Micropay API interface call failure, return_msg : " + returnMsg);
                throw new WxPayException("请检查网络，微信支付失败" );
            }
                ...
                result.GetValue("result_code").ToString() == "SUCCESS")
            {
               ...
               // return result.ToPrintStr();
                Log.Info("MicroPay", result.ToPrintStr());
                return;
            }
            ...
            result.GetValue("err_code").ToString() != "SYSTEMERROR")
            {
                ...
                Log.Info("MicroPay", result.ToPrintStr());
                throw new WxPayException("微信支付失败"+result.ToPrintStr());
            }
            ...
            if (!Cancel(out_trade_no))
            {
                ...
                throw new WxPayException("微信支付失败，订单撤销失败！");
            }
            else {
                throw new WxPayException("微信支付失败，订单已撤销");
            }
