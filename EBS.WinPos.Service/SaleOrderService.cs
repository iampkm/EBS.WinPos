...
namespace EBS.WinPos.Service
{
    public class SaleOrderService
    {
        ...
        public void WechatPay(int orderId, string payBarCode)
        {
            ...
            var model = _db.Orders.FirstOrDefault(n => n.Id == orderId);
            if (model == null) { throw new Exception("订单不存在"); }
            if (model.RealAmount <= 0) { throw new Exception("订单金额不能为0"); }
            // 发起微信支付
            string data = JsonHelper.GetJson(new { barcode = payBarCode, orderId = model.Code, paymentAmt = model.RealAmount.ToString("F2"), systemId = "2" });
            ...
            {
                //支付成功
                model.FinishPaid(PaymentWay.WechatPay);
                _db.SaveChanges();
            }
            else {
